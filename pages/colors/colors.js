    ...
    return new Promise((resolve) => {
      chrome.tabs.sendMessage(tab.id, { action: "getColors" }, (response) => {
        if (chrome.runtime.lastError) {
          console.error(
            "Error communicating with content e script:",
        ...
        try {
          ...
          if (!Array.isArray(response.colors)) {
            console.error(
              "Expected colors to be an array, got:",
              response.colors
            );
            resolve(new Map());
            return;
...
function generateNumberedColorName(color, colors, familyName) {
  ...
  const baseName = familyName.endsWith("s")
        ...
        (response) => {
          ...
          try {
            ...
            if (!Array.isArray(response.gradients)) {
              console.error(
                "Expected gradients to be an array, got:",
                response.gradients
              );
              resolve([]);
              return;
  ...
  try {
    ...
    sortedGradients.forEach(([normalizedKey, gradientData]) => {
      ...
      if (gradientData && typeof gradientData.original === "string") {
        const card = createGradientCard(
          gradientData.original,
          gradientData.count
        );
...
function createGradientCard(gradient, count) {
  ...
  if (typeof gradient !== "string") {
    console.error(
      "Invalid gradient type passed to createGradientCard:",
      typeof gradient,
      gradient
    );
  ...
  const gradientColors =
    gradient.match(/rgba?\([^)]+\)|#[0-9a-f]{3,8}/gi) || [];
...
function getGradientName(gradient) {
  ...
  if (colors.length === 0) {
    const potentialColors = gradient
      .split(/,|\s+/)
...
function displayExport() {
  ...
  const sortedColors = Array.from(allColors.entries()).sort(
    (a, b) => b[1] - a[1]
  );
...
function setupTabs() {
  ...
  const updateColors = () => {
    ...
    // Sort colors by usage count
    const sortedColors = Array.from(colorGroup.entries()).sort(
      (a, b) => b[1] - a[1]
    );
    ...
    familyOptions.forEach((option) => {
      ...
      const hasColors =
        colorFamilies.has(option.value) &&
        colorFamilies.get(option.value).length > 0;
...
function createFamilySection(familyName, colors) {
  ...
  if (familyName === "Grayscale") {
    ...
    if (nearWhites.length > 0) {
      const totalNearWhiteCount = nearWhites.reduce(
        (sum, [, count]) => sum + count,
        0
      );
      ...
      const countEl = document.createElement("div");
      countEl.className = "color-count";
      countEl.textContent = `Used ${totalNearWhiteCount} time${
        totalNearWhiteCount > 1 ? "s" : ""
      } total`;
