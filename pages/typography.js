...
function generateFontStyles(fontData) {
  ...
  const previewText = document.querySelector(
    `.font-preview-text[style*="font-family: '${fontData.family}'"]`
  );
  const currentStyles = previewText
    ? window.getComputedStyle(previewText)
    : null;
  ...
  const lineHeight = currentStyles
...
function normalizeFontSize(fontSize, element, styleSheet) {
  ...
  if (fontSize === "inherit" && element) {
    ...
    fontSize =
      resolveInheritedValue("fontSize", element, computedStyle) || "16px";
  }
...
async function getWebpageTypography() {
  try {
    ...
    try {
      ...
      throw new Error(
        "Could not get typography data. Please refresh the page and try again."
      );
    }
  } catch (error) {
...
function processTypographyData(rawData) {
  ...
  return Object.values(rawData)
    ...
    .map((item) => {
      ...
      const fontFamily = item.styles.fontFamily
        .split(",")[0]
        .replace(/['"]/g, "")
        .trim();
...
const createFontCard = (fontData) => {
  ...
  displayWeightName =
    displayWeightName.charAt(0).toUpperCase() + displayWeightName.slice(1);
  ...
  card.innerHTML = `
    <div class="font-preview" style="background-image: ${getPatternStyle(
      category
  ...
  controls.forEach((control) => {
    control.addEventListener("click", (e) => {
      ...
      const currentSizePx = parseFloat(
        window.getComputedStyle(previewText).fontSize
      );
      const newSizePx =
        control.dataset.action === "increase"
          ? currentSizePx + 2
          : currentSizePx - 2;
      if (newSizePx >= 8 && newSizePx <= 48) {
        // Limit size range
...
async function updateTypographyDisplay(weight) {
  ...
  try {
    ...
    const sortedFamilies = Object.entries(fontsByFamily).sort(
      ([, fontsA], [, fontsB]) => fontsB.length - fontsA.length
    );
...
function createScalePreview(size, name, text = "Aa") {
  ...
  const unit = getCurrentUnit();
  const displaySize =
    unit === "rem" ? `${(size / 16).toFixed(3)}rem` : `${size}px`;
...
function displayTypeScale() {
  ...
  const exportContent =
    format === "css" ? generateScaleCSS(scale) : generateTailwindConfig(scale);
